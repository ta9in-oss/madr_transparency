...
export function LanguageSwitcher({ current, labels }: Props) {
  ...
  function selectLang(lang: Lang) {
    ...
    // Strip any current lang prefix
    if (segments.length > 0 && (LANGS as string[]).includes(segments[0])) {
      segments.shift();
    }
    // For Arabic (default), no prefix
    if (lang === 'ar') {
      url.pathname = '/' + segments.join('/') || '/';
    } else {
      url.pathname = '/' + lang + (segments.length ? '/' + segments.join('/') : '');
    }
